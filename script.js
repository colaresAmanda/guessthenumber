...
function render_screen_IV(lang) {
  ...
  if (!localStorage.getItem("randomNumber")) {
    game.randomNumber = get_random_number(game.minNumber, game.maxNumber);
  ...
  getAttemptForm.addEventListener("submit", (event) => {
    ...
    if (
      attemptInput.value.trim().length === 0 ||
      isNaN(parseInt(attemptInput.value)) ||
      attemptInput.value < game.minNumber ||
      attemptInput.value > game.maxNumber
    ) {
